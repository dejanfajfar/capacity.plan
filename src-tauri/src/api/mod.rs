...
pub async fn fetch_public_holidays(
    country_code: &str,
    year: i32,
) -> Result<Vec<NagerDateHoliday>, String> {
    info!("Fetching public holidays for {} in {}", country_code, year);

    let url = format!(
        "{}/PublicHolidays/{}/{}",
        NAGER_DATE_BASE_URL, year, country_code
    );
    ...
    let holidays = response
        ...
        .map_err(|e| {
            error!(
                "Failed to parse holidays response for {} ({}): {}",
                country_code, year, e
            );
            format!("Parse error: {}", e)
        })?;
    ...
    info!(
        "Successfully fetched {} holidays for {} ({})",
        holidays.len(),
        country_code,
        year
    );
